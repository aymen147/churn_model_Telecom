...
class FilesCompleter(BaseCompleter):
    ...
    def __call__(self, prefix, **kwargs):
        ...
        if self.allowednames:
            if self.directories:
                ...
                files = _call(
                    [
                        "bash",
                        "-c",
                        "bind; compgen -A directory -- '{p}'".format(p=prefix),
                    ],
            ...
            for x in self.allowednames:
                completion += _call(
                    [
                        "bash",
                        "-c",
                        "bind; compgen -A file -X '!*.{0}' -- '{p}'".format(
                            x, p=prefix
                        ),
                    ],
                    stderr=subprocess.DEVNULL,
            ...
            completion += _call(
                ["bash", "-c", "bind; compgen -A file -- '{p}'".format(p=prefix)],
                stderr=subprocess.DEVNULL,
            )
            anticomp = _call(
                ["bash", "-c", "bind; compgen -A directory -- '{p}'".format(p=prefix)],
                stderr=subprocess.DEVNULL,
            )

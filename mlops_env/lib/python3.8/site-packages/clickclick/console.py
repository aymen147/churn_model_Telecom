...
class Action:
    ...
    def fatal_error(self, msg, **kwargs):
        self._suppress_exception = (
            True  # Avoid printing "EXCEPTION OCCURRED: -1" on exit
        )
...
def print_table(cols, rows, styles=None, titles=None, max_column_widths=None):
    ...
    for row in rows:
        for col in cols:
            val = row.get(col)
            colwidths[col] = min(
                max(colwidths[col], len(format(col, val))),
                max_column_widths.get(col, 1000),
            )

    for i, col in enumerate(cols):
        click.secho(
            ("{:" + str(colwidths[col]) + "}").format(
                titles.get(col, col.title().replace("_", " "))
            ),
            nl=False,
            fg="black",
            bg="white",
    ...
    for row in rows:
        for col in cols:
            ...
            click.secho(
                ("{:" + align + str(colwidths[col]) + "}").format(val),
                nl=False,
                **style
            )
            click.echo(" ", nl=False)
...
def choice(prompt, options, default=None):
    ...
    while True:
        selection = click.prompt(
            "Please select (1-{})".format(len(options)),
            type=int,
...
class UrlType(click.types.ParamType):
    ...
    def convert(self, value, param, ctx):
        ...
        if self.allowed_schemes and url.scheme not in self.allowed_schemes:
            self.fail(
                '"{}" is not one of the allowed URL schemes ({})'.format(

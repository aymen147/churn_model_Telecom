...
def external_browser(cfg: "Config") -> Optional[CredentialsProvider]:
    ...
    if credentials:
        ...
        # TODO: We should ideally use more specific exceptions.
            ...
            logger.warning(
                f"Failed to refresh cached token: {e}. Initiating new OAuth login flow"
            )
...
@oauth_credentials_strategy(
    "azure-client-secret", ["is_azure", "azure_client_id", "azure_client_secret"]
)
...
def github_oidc_azure(cfg: "Config") -> Optional[CredentialsProvider]:
    ...
    logger.info(
        "Configured AAD token for GitHub Actions OIDC (%s)", cfg.azure_client_id
    )
    params = {
...
def google_credentials(cfg: "Config") -> Optional[CredentialsProvider]:
    ...
    gcp_credentials = service_account.Credentials.from_service_account_info(
        info=account_info, scopes=GcpScopes
    )
...
def google_id(cfg: "Config") -> Optional[CredentialsProvider]:
    ...
    def refreshed_headers() -> Dict[str, str]:
        ...
        if cfg.is_account_client:
            ...
            headers["X-Databricks-GCP-SA-Access-Token"] = (
                gcp_impersonated_credentials.token
            )
        return headers
...
def _run_subprocess(
    popenargs, input=None, capture_output=True, timeout=None, check=False, **kwargs
...
class AzureCliTokenSource(CliTokenSource):
    ...
    def for_resource(cfg: "Config", resource: str) -> "AzureCliTokenSource":
        ...
        if subscription is not None:
            token_source = AzureCliTokenSource(
                resource, subscription=subscription, tenant=cfg.azure_tenant_id
            )
            try:
                ...
                logger.warning(
                    "Failed to get token for subscription. Using resource only token."
                )

        token_source = AzureCliTokenSource(
            resource, subscription=None, tenant=cfg.azure_tenant_id
        )
        token_source.token()
        return token_source
...
def azure_cli(cfg: "Config") -> Optional[CredentialsProvider]:
    ...
    try:
        token_source = AzureCliTokenSource.for_resource(
        ...
        logger.debug(
            "This may happen if you are attempting to login to a dev or staging workspace"
        )
        return None
    ...
    if not token_source.is_human_user():
        try:
            ...
            mgmt_token_source = AzureCliTokenSource.for_resource(
                cfg, management_endpoint
            )
        except Exception as e:
            logger.debug(
                f"Not including service management token in headers", exc_info=e
            )
            mgmt_token_source = None

    _ensure_host_present(
        cfg, lambda resource: AzureCliTokenSource.for_resource(cfg, resource)
    )
...
class ModelServingAuthProvider:
    _MODEL_DEPENDENCY_OAUTH_TOKEN_FILE_PATH = (
        "/var/credentials-secret/model-dependencies-oauth-token"
    )
    ...
    def should_fetch_model_serving_environment_oauth(self) -> bool:
        ...
        return is_in_model_serving_env == "true" and os.path.isfile(
            self._MODEL_DEPENDENCY_OAUTH_TOKEN_FILE_PATH
        )
...
def model_serving_auth(cfg: "Config") -> Optional[CredentialsProvider]:
    try:
        ...
        if (
            not model_serving_auth_provider.should_fetch_model_serving_environment_oauth()
        ):
        ...
        logger.warning(
            "Unable to get auth from Databricks Model Serving Environment", exc_info=e
        )
        return None
    ...
    def __call__(self, cfg: "Config") -> CredentialsProvider:
        for provider in self._auth_providers:
            ...
            if cfg.auth_type and auth_type != cfg.auth_type:
                ...
                logger.debug(
                    f"Ignoring {auth_type} auth, because {cfg.auth_type} is preferred"
                )
                continue

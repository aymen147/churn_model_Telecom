...
def compile(pattern, options=None):
    if isinstance(pattern, _Regexp):
        if options:
            raise error(
                "pattern is already compiled, so " "options may not be specified"
            )
...
class _Regexp(object):
    ...
    def _match(self, anchor, text, pos=None, endpos=None):
        ...
        if isinstance(text, str):
            ...
            last_offset = 0
            while True:
                spans = self._regexp.Match(
                    anchor, encoded_text, encoded_pos, encoded_endpos
                )
                ...
                for offset in offsets:
                    decoded_offsets[offset] = decoded_offsets[
                        last_offset

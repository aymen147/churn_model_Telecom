...
class linePlusBarChart(TemplateMixin, NVD3Chart):
    ...
    def __init__(self, **kwargs):
        ...
        self.yaxis1_format = kwargs.get(
            "yaxis1_format", "function(d) { return d3.format(',f')(d) }"
        )
        self.yaxis2_format = kwargs.get(
            "yaxis2_format", "function(d) { return d3.format(',f')(d) }"
        )

        if kwargs.get("x_is_date", False):
            self.set_date_flag(True)
            self.create_x_axis(
                "xAxis", format=kwargs.get("x_axis_format", "%d %b %Y %H %S"), date=True
            )
            self.create_x_axis(
                "x2Axis",

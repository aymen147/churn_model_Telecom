...
class NVD3Chart(object):
        ...
        self.header_css = [
            ...
            for h in (
                (
                    ...
                    if self.remote_js_assets
                    else self.assets_directory + "nvd3/src/nv.d3.css"
                ),
            )
        ]

        self.header_js = [
            '<script src="%s"></script>' % h
            for h in (
                (
                    "https://cdnjs.cloudflare.com/ajax/libs/d3/3.5.17/d3.min.js"
                    if self.remote_js_assets
                    else self.assets_directory + "d3/d3.min.js"
                ),
                (
                    "https://cdnjs.cloudflare.com/ajax/libs/nvd3/1.8.6/nv.d3.min.js"
                    if self.remote_js_assets
                    else self.assets_directory + "nvd3/nv.d3.min.js"
                ),
            )
        ]
    ...
    def create_x_axis(
        self, name, label=None, format=None, date=False, custom_format=False
    ):
        """Create X-axis"""
        axis = {}
        ...
        if date:
            ...
            axis["tickFormat"] = (
                "function(d) { return d3.time.format('%s')"
                "(new Date(parseInt(d))) }\n"
                "" % self.dateformat
            )
...
def _main():
    ...
    parser = OptionParser(
        usage=usage,
        version=("python-nvd3 - Charts generator with " "nvd3.js and d3.js"),
    )
    parser.add_option(
        "-q",
        "--quiet",

...
class AirflowSecurityManagerV2(LoggingMixin):
    ...
    def has_access(
        ...
        is_authorized_method = self._get_auth_manager_is_authorized_method(
            resource_name
        )
    ...
    def _auth_manager_is_authorized_map(
        ...
        def get_connection_id(resource_pk):
            ...
            connection = session.scalar(
                select(Connection).where(Connection.id == resource_pk).limit(1)
            )
            if not connection:
        ...
        def get_dag_id_from_dagrun_id(resource_pk):
            ...
            dagrun = session.scalar(
                select(DagRun).where(DagRun.id == resource_pk).limit(1)
            )
            if not dagrun:
        ...
        def _is_authorized_dag(entity_=None, details_func_=None):
            return lambda action, resource_pk, user: auth_manager.is_authorized_dag(
                ...
                details=(
                    DagDetails(id=details_func_(resource_pk)) if details_func_ else None
                ),
                user=user,
            )
    ...
    def _get_auth_manager_is_authorized_method(
        self, fab_resource_name: str
    ) -> Callable:
        is_authorized_method = self._auth_manager_is_authorized_map.get(
            fab_resource_name
        )

...
class TracebackSessionForTests:
    ...
    def is_called_from_test_code(self) -> tuple[bool, traceback.FrameSummary | None]:
        ...
        self.traceback = traceback.extract_stack()
        if any(
            filename.endswith("_pytest/fixtures.py")
            for filename, _, _, _ in self.traceback
        ):
        ...
        if any(
            ...
            or (
                filename.startswith(AIRFLOW_TESTS_PATH)
                and name in ("setup_method", "teardown_method")
            )
            for filename, _, name, _ in airflow_frames
        ):
            # This is a fixture call or testing utilities
            return True, None
        if len(airflow_frames) >= 2 and airflow_frames[-2].filename.startswith(
            AIRFLOW_TESTS_PATH
        ):
            # Let's look at what we are calling directly from the test code
            current_filename, current_method_name = (
                airflow_frames[-1].filename,
                airflow_frames[-1].name,
            )
...
DEFAULT_ENGINE_ARGS = {
    "postgresql": {
        ...
        (
            ...
            if is_sqlalchemy_v1()
            else "insertmanyvalues_page_size"
        ): 10000,

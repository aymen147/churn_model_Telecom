    ...
    return {
        ...
        for x in session.execute(
            select(
                ...
                case((func.count() == 1, func.max(DatasetModel.uri)), else_="").label(
                    "uri"
                ),
            ...
            .join(
                DatasetModel, DatasetModel.id == DagScheduleDatasetReference.dataset_id
            )
...
class DAG(LoggingMixin):
    ...
    def __init__(
        ...
        if tags and any(len(tag) > TAG_MAX_LEN for tag in tags):
            raise AirflowException(
                f"tag cannot be longer than {TAG_MAX_LEN} characters"
            )
        ...
        if not at_most_one(*scheduling_args):
            raise ValueError(
                "At most one allowed for args 'schedule_interval', 'timetable', and 'schedule'."
            )
        ...
        if isinstance(schedule, Timetable):
            ...
            if not all(isinstance(x, (Dataset, DatasetAlias)) for x in schedule):
                raise ValueError(
                    "All elements in 'schedule' should be datasets or dataset aliases"
                )
            ...
            if packaging_version.parse(FAB_VERSION) >= packaging_version.parse("1.3.0"):
                ...
                if permissions.RESOURCE_DAG in updated_access_control[role]:
                    updated_access_control[role][permissions.RESOURCE_DAG] = {
                        ...
                        for perm in updated_access_control[role][
    ...
    def date_range(
        ...
        message += (
            " Please use `DAG.iter_dagrun_infos_between(..., align=False)` instead."
        )
        warnings.warn(message, category=RemovedInAirflow3Warning, stacklevel=2)
        if end_date is None:
        ...
        it = self.iter_dagrun_infos_between(
            start_date, pendulum.instance(coerced_end_date), align=False
        )
    ...
    def get_run_dates(self, start_date, end_date=None) -> list:
        ...
        return [
            info.logical_date
            for info in self.iter_dagrun_infos_between(earliest, latest)
        ]
    ...
    def normalized_schedule_interval(self) -> ScheduleInterval:
        ...
        if (
            ...
            and self.schedule_interval in cron_presets
        ):
            _schedule_interval: ScheduleInterval = cron_presets.get(
    ...
    def create_dagrun(
        ...
        if run_id:  # Infer run_type from run_id if needed.
            ...
            if run_type is None:
                ...
                run_type == DagRunType.MANUAL and inferred_run_type != DagRunType.MANUAL
            ):
    ...
    def bulk_write_to_db(
        ...
        if any(dag.timetable.can_be_scheduled for dag in dags):
            ...
            num_active_runs = DagRun.active_runs_of_dags(
                dag_ids=existing_dags, session=session
            )
        ...
        for orm_dag in sorted(orm_dags, key=lambda d: d.dag_id):
            ...
            for owner_name, owner_link in dag.owner_links.items():
                dag_owner_orm = DagOwnerAttributes(
                    dag_id=dag.dag_id, owner=owner_name, link=owner_link
                )
        ...
        dag_references: dict[
            str, set[tuple[Literal["dataset", "dataset-alias"], str]]
        ] = defaultdict(set)
        ...
        for dag in dags:
            ...
            if not (dataset_condition := dag.timetable.dataset_condition):
                ...
                for dataset_alias in dataset_condition.iter_dataset_aliases():
                    dag_references[dag.dag_id].add(
                        ("dataset-alias", dataset_alias.name)
                    )
                    input_dataset_alias_models.add(
                        DatasetAliasModel.from_public(dataset_alias)
                    )

            curr_outlet_references = (
                curr_orm_dag and curr_orm_dag.task_outlet_dataset_references
            )
        ...
        dataset_manager.create_datasets(
            dataset_models=new_dataset_models, session=session
        )
        ...
        all_datasets_alias_models = (
            input_dataset_alias_models | outlet_dataset_alias_models
        )
        ...
        if all_datasets_alias_models:
            ...
            stored_dataset_alias_models = {
                ...
                for dsa_m in session.scalars(
                    select(DatasetAliasModel).where(
                        DatasetAliasModel.name.in_(all_dataset_alias_names)
                    )
                ).fetchall()
            }
            ...
            if stored_dataset_alias_models:
                new_dataset_alias_models = {
                    dataset_alias_model
                    for dataset_alias_model in all_datasets_alias_models
                    if dataset_alias_model.name
                    not in stored_dataset_alias_models.keys()
                }
            else:
        ...
        for dag_id, base_dataset_list in dag_references.items():
            dag_refs_needed = {
                (
                    DagScheduleDatasetReference(
                        dataset_id=stored_dataset_models[base_dataset_identifier].id,
                        dag_id=dag_id,
                    )
                    if base_dataset_type == "dataset"
                    else DagScheduleDatasetAliasReference(
                        alias_id=stored_dataset_alias_models[
                            base_dataset_identifier
                        ].id,
                        dag_id=dag_id,
                    )
                )
        ...
        for (dag_id, task_id), uri_list in outlet_references.items():
            task_refs_needed = {
                TaskOutletDatasetReference(
                    dataset_id=stored_dataset_models[uri].id,
                    dag_id=dag_id,
                    task_id=task_id,
                )
                for uri in uri_list
            ...
            task_refs_to_add = {
                x for x in task_refs_needed if x not in task_refs_stored
            }
        ...
        for dag in dags:
            cls.bulk_write_to_db(
                dag.subdags, processor_subdir=processor_subdir, session=session
            )
    ...
    def sync_to_db(self, processor_subdir: str | None = None, session=NEW_SESSION):
        ...
        self.bulk_write_to_db(
            [self], processor_subdir=processor_subdir, session=session
        )
...
class DagModel(Base):
    ...
    @provide_session
    def get_current(
        cls, dag_id: str, session=NEW_SESSION
    ...
    @provide_session
    def set_is_paused(
        self, is_paused: bool, including_subdags: bool = True, session=NEW_SESSION
    ) -> None:
        """
    ...
    def dags_needing_dagruns(
        ...
        for ser_dag in ser_dags:
            ...
            if not dag_ready(
                dag_id, cond=ser_dag.dag.timetable.dataset_condition, statuses=statuses
            ):
                del by_dag[dag_id]
    ...
    def calculate_dagrun_date_fields(
        ...
        if isinstance(last_automated_dag_run, datetime):
            ...
            last_automated_data_interval = dag.infer_automated_data_interval(
    ...
    def get_dataset_triggered_next_run_info(
        ...
        if self.schedule_interval != "Dataset":
            return None
        return get_dataset_triggered_next_run_info([self.dag_id], session=session)[
            self.dag_id
        ]

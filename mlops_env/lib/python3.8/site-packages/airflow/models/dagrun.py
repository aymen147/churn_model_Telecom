...
class DagRun(Base, LoggingMixin):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "dag_id", "execution_date", name="dag_run_dag_id_execution_date_key"
        ),
    ...
    def next_dagruns_to_examine(
        ...
        if state == DagRunState.QUEUED:
            ...
            query = query.outerjoin(
                running_drs, running_drs.c.dag_id == DagRun.dag_id
            ).where(
        ...
        return session.scalars(
            with_row_locks(
                query.limit(max_number), of=cls, session=session, skip_locked=True
            )
        )
    ...
    def find(
        ...
        if execution_start_date and execution_end_date:
            qry = qry.where(
                cls.execution_date.between(execution_start_date, execution_end_date)
            )
    ...
    def update_state(
        ...
        if not unfinished.tis and any(
            ...
            if execute_callbacks:
                ...
                callback = DagCallbackRequest(
                    ...
                    processor_subdir=(
                        None if dag_model is None else dag_model.processor_subdir
                    ),
                    msg="task_failure",
                )
        ...
        # if all leaves succeeded and no unfinished tasks, the run succeeded
            ...
            if execute_callbacks:
                dag.handle_callback(
                    self, success=True, reason="success", session=session
                )
        ...
        if self._state == DagRunState.FAILED or self._state == DagRunState.SUCCESS:
            ...
            with Trace.start_span_from_dagrun(dagrun=self) as span:
                ...
                if span.is_recording():
                    span.add_event(
                        name="queued", timestamp=datetime_to_nano(self.queued_at)
                    )
                    span.add_event(
                        name="started", timestamp=datetime_to_nano(self.start_date)
                    )
                    span.add_event(
                        name="ended", timestamp=datetime_to_nano(self.end_date)
                    )
    ...
    def _get_ready_tis(
        ...
        def _expand_mapped_task_if_needed(ti: TI) -> Iterable[TI] | None:
            ...
            try:
                expanded_tis, _ = ti.task.expand_mapped_task(
    ...
    def _check_for_removed_or_restored_tasks(
        ...
        for ti in tis:
            ...
            try:
                ...
                try:
                    total_length = task.get_mapped_ti_count(
                        self.run_id, session=session
                    )
                except NotFullyPopulated:
    ...
    def schedule_tis(
            ...
            for schedulable_ti_ids_chunk in schedulable_ti_ids_chunks:
                count += session.execute(
                    ...
                    .where(
                        ...
                        TI.run_id == self.run_id,
                        tuple_in_condition(
                            (TI.task_id, TI.map_index), schedulable_ti_ids_chunk
                        ),
                    )
                    .values(
        ...
        if dummy_ti_ids:
            dummy_ti_ids_chunks = chunks(
                dummy_ti_ids, max_tis_per_query or len(dummy_ti_ids)
            )
            for dummy_ti_ids_chunk in dummy_ti_ids_chunks:
                count += session.execute(
                    ...
                    .where(
                        ...
                        TI.run_id == self.run_id,
                        tuple_in_condition(
                            (TI.task_id, TI.map_index), dummy_ti_ids_chunk
                        ),
                    )
                    .values(

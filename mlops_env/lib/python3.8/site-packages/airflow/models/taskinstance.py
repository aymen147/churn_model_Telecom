...
def _run_raw_task(
    ...
    with set_current_task_instance_session(session=session):
        ...
        context = ti.get_template_context(
            ignore_param_exceptions=False, session=session
        )
        ...
        try:
            ...
            _run_finished_callback(
                callbacks=ti.task.on_skipped_callback, context=context
            )
            TaskInstance.save_to_db(ti=ti, session=session)
    ...
    for ti in tis:
        ...
        if not task.is_teardown:
            if ti.state == TaskInstanceState.RUNNING:
                log.info(
                    "Forcing task %s to fail due to dag's `fail_stop` setting",
                    ti.task_id,
                )
                ti.error(session)
            else:
...
def clear_task_instances(
    ...
    for ti in tis:
        ...
        if ti.state == TaskInstanceState.RUNNING:
            ...
            ti.clear_next_method_args()
            session.merge(ti)
        task_id_by_key[ti.dag_id][ti.run_id][ti.map_index][ti.try_number].add(
            ti.task_id
        )
            ...
            if task_to_execute.multiple_outputs:
                ...
                for key, value in xcom_value.items():
                    task_instance.xcom_push(
                        key=key, value=value, session=session_or_null
                    )
            task_instance.xcom_push(
...
def _get_template_context(
    *,
    ...
    def get_triggering_events() -> dict[str, list[DatasetEvent | DatasetEventPydantic]]:
        ...
        triggering_events: dict[str, list[DatasetEvent | DatasetEventPydantic]] = (
            defaultdict(list)
        )
...
class TaskInstance(Base, LoggingMixin):
    ...
    __table_args__ = (
        ...
        PrimaryKeyConstraint(
            "dag_id", "task_id", "run_id", "map_index", name="task_instance_pkey"
        ),
        ForeignKeyConstraint(
            [trigger_id],
            ["trigger.id"],
            ...
            if execution_date and not timezone.is_localized(execution_date):
                ...
                if self.task.has_dag():
                    ...
                    execution_date = timezone.make_aware(
                        execution_date, self.task.dag.timezone
                    )
                else:
    ...
    def get_previous_start_date(
        ...
        :param session: SQLAlchemy ORM Session
        """
        return _get_previous_start_date(
            task_instance=self, state=state, session=session
        )
    ...
    def _register_dataset_changes(
        ...
        dataset_tuple_to_alias_names_mapping: dict[tuple[str, frozenset], set[str]] = (
            defaultdict(set)
        )
        for obj in self.task.outlets or []:
            ...
            if isinstance(obj, Dataset):
                ...
                for dataset_alias_event in events[obj].dataset_alias_events:
                    ...
                    dataset_tuple_to_alias_names_mapping[
                        (dataset_uri, frozen_extra)
                    ].add(dataset_alias_name)

        dataset_objs_cache: dict[str, DatasetModel] = {}
        for (
            uri,
            extra_items,
        ), alias_names in dataset_tuple_to_alias_names_mapping.items():
            if uri not in dataset_objs_cache:
                dataset_obj = session.scalar(
                    select(DatasetModel).where(DatasetModel.uri == uri).limit(1)
                )
            ...
            if not dataset_obj:
                ...
                dataset_manager.create_datasets(
                    dataset_models=[dataset_obj], session=session
                )
            ...
            for alias in alias_names:
                alias_obj = session.scalar(
                    ...
                    .where(DatasetAliasModel.name == alias)
                    .limit(1)
                )
...
class TaskInstanceNote(TaskInstanceDependencies):
    ...
    user_id = Column(
        Integer,
        ForeignKey("ab_user.id", name="task_instance_note_user_fkey"),
        nullable=True,
    )
    task_id = Column(StringID(), primary_key=True, nullable=False)
    dag_id = Column(StringID(), primary_key=True, nullable=False)

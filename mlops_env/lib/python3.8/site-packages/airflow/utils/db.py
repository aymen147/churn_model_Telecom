...
def _get_flask_db(sql_database_uri):
    ...
    AirflowDatabaseSessionInterface(
        app=flask_app, db=db, table="session", key_prefix=""
    )
    return db
...
def check_table_for_duplicates(
    *, session: Session, table_name: str, uniqueness: list[str], version: str
    ...
    try:
        ...
        log.warning(
            "Found %s duplicates in table %s.  Will attempt to move them.",
            dupe_count,
            table_name,
        )
        ...
        _move_duplicate_data_to_new_table(
            ...
            target_table_name=_format_airflow_moved_table_name(
                table_name, version, "duplicates"
            ),
        )
    except (exc.OperationalError, exc.ProgrammingError):
...
def check_conn_type_null(session: Session) -> Iterable[str]:
    ...
    try:
        n_nulls = session.scalars(
            select(Connection.conn_id).where(Connection.conn_type.is_(None))
        ).all()
...
def check_run_id_null(session: Session) -> Iterable[str]:
    ...
    invalid_dagrun_count = session.scalar(
        select(func.count(dagrun_table.c.id)).where(invalid_dagrun_filter)
    )
    if invalid_dagrun_count > 0:
        dagrun_dangling_table_name = _format_airflow_moved_table_name(
            dagrun_table.name, "2.2", "dangling"
        )
...
def _create_table_as(
    *,
    ...
    if dialect_name == "mysql":
        ...
        session.execute(
            text(f"CREATE TABLE {target_table_name} LIKE {source_table_name}")
        )
        session.execute(
            text(
...
def _move_dangling_data_to_new_table(
    ...
    target_table = source_table.to_metadata(
        source_table.metadata, name=target_table_name
    )
    ...
    if not first_moved_row:
        ...
        if dialect_name == "sqlite":
            ...
            delete = source_table.delete().where(
                tuple_(*pk_cols).in_(
                    session.select(*target_table.primary_key.columns).subquery()
                )
            )
        else:
            delete = source_table.delete().where(
...
def _move_duplicate_data_to_new_table(
    ...
    query = (
        select(
            *(source_table.c[x.name].label(str(x.name)) for x in source_table.columns)
        )
...
def check_bad_references(session: Session) -> Iterable[str]:
    ...
    metadata = reflect_tables(
        [*(x[0] for x in models_list), DagRun, TaskInstance], session
    )
    ...
    for model, change_version, bad_ref_cfg in models_list:
        ...
        dangling_table_name = _format_airflow_moved_table_name(
            source_table.name, change_version, "dangling"
        )
...
def _revisions_above_min_for_offline(config, revisions) -> None:
    ...
    min_version, min_revision = (
        ("2.2.0", "7b2661a43ba3") if dbname == "mssql" else ("2.0.0", "e959f08ac86c")
    )
...
def upgradedb(
    *,
    ...
    if (
        not to_revision
        and not _get_current_revision(session=session)
        and not use_migration_files
    ):

...
class FabAuthManager(BaseAuthManager):
    ...
    def get_api_endpoints(self) -> None | Blueprint:
        ...
        )  # this is airflow/auth/managers/fab/
    ...
    def is_authorized_dataset(
        ...
        *,
        ...
        return self._is_authorized(
            method=method, resource_type=RESOURCE_DATASET, user=user
        )
    ...
    def is_authorized_pool(
        ...
        *,
        ...
        return self._is_authorized(
            method=method, resource_type=RESOURCE_POOL, user=user
        )
    ...
    def get_permitted_dag_ids(
        ...
        *,
        ...
        if not self.is_logged_in():
            ...
            if (
                "GET" in methods and self.is_authorized_dag(method="GET", user=user)
            ) or ("PUT" in methods and self.is_authorized_dag(method="PUT", user=user)):
            ...
            for permission in role.permissions:
                ...
                if (
                    ...
                    if resource == permissions.RESOURCE_DAG:
                        return {
                            dag.dag_id
                            for dag in session.execute(select(DagModel.dag_id))
                        }
        ...
        return {
            dag.dag_id
            for dag in session.execute(
                select(DagModel.dag_id).where(DagModel.dag_id.in_(resources))
            )
        }

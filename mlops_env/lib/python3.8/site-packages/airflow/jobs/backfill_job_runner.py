...
class BackfillJobRunner(BaseJobRunner, LoggingMixin):
    ...
    def _update_counters(self, ti_status: _DagRunTaskStatus, session: Session) -> None:
        ...
        ti_primary_key_to_ti_key = {
            ti_key.primary: ti_key for ti_key in ti_status.running.keys()
        }
    ...
    def _manage_executor_state(
        ...
        running_dict = {
            (ti.dag_id, ti.task_id, ti.run_id, ti.map_index): ti
            for ti in refreshed_running_tis
        }
        need_refresh = False
        ...
        for key, value in buffered_events:
            ...
            if ti_key not in running_dict:
                self.log.warning(
                    "%s state %s not in running=%s", key, state, running.values()
                )
                continue
            ...
            def _iter_task_needing_expansion() -> Iterator[AbstractOperator]:
                ...
                for node in self.dag.get_task(
                    ti.task_id, include_subdags=True
            ...
            if ti.state not in self.STATES_COUNT_AS_RUNNING:
                ...
                for node in _iter_task_needing_expansion():
                    new_tis, num_mapped_tis = node.expand_mapped_task(
    ...
    def _get_dag_run(
        ...
        respect_dag_max_active_limit = bool(
            dag.timetable.can_be_scheduled and not dag.is_subdag
        )
        ...
        *,
        ...
        while (ti_status.to_run or ti_status.running) and not ti_status.deadlocked:
            ...
            def _per_task_process(key, ti: TaskInstance, session):
                ...
                executor = ExecutorLoader.load_executor(
                    str(ti.executor) if ti.executor else None
                )
                ...
                if ti.state == TaskInstanceState.UP_FOR_RESCHEDULE:
                    self.log.debug(
                        "Task instance %s reschedule period not expired yet", ti
                    )
                    if key in ti_status.running:
                        ti_status.running.pop(key)
                    ti_status.to_run[key] = ti
                ...
                for task in self.dag.topological_sort(include_subdag_tasks=True):
                    for key, ti in list(ti_status.to_run.items()):
                        ...
                        for i in range(max_attempts):
                            ...
                            pool = session.scalar(
                                select(models.Pool)
                                .where(models.Pool.pool == task.pool)
                                .limit(1)
                            )
                            if not pool:
            ...
            for executor in self.job.executors:
                ...
                for (
                    ...
                    new_mapped_tis,
                    max_map_index,
                ) in self._manage_executor_state(ti_status.running, executor, session):

                    def to_keep(key: TaskInstanceKey) -> bool:
                        if (
                            key.dag_id != node.dag_id
                            or key.task_id != node.task_id
                            or key.run_id != run_id
                        ):
    ...
    @provide_session
    def _collect_errors(
        self, ti_status: _DagRunTaskStatus, session: Session = NEW_SESSION
    ) -> Iterator[str]:
    ...
    def _execute_dagruns(
        ...
        for dagrun_info in dagrun_infos:
            for dag in self._get_dag_with_subdags():
                ...
                if dag_run is not None:
                    tis_map = self._task_instances_for_dag_run(
                        dag, dag_run, session=session
                    )
        ...
        if tis_missing_executor:
            raise UnknownExecutorException(
                ...
                + "\n".join(
                    [
                        f"  {ti.task_id}: {ti.run_id} (executor: {ti.executor})"
                        for ti in tis_missing_executor
                    ]
                )
            )
    ...
    def _execute(self, session: Session = NEW_SESSION) -> None:
        ...
        dagrun_infos = list(
            self.dag.iter_dagrun_infos_between(dagrun_start_date, dagrun_end_date)
        )
    ...
    def reset_state_for_orphaned_tasks(
        ...
        reset_tis = helpers.reduce_in_chunks(
            query, tis_to_reset, [], self.job.max_tis_per_query
        )

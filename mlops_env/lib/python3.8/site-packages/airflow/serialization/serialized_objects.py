...
def encode_dataset_condition(var: BaseDataset) -> dict[str, Any]:
    ...
    if isinstance(var, DatasetAll):
        return {
            "__type": DAT.DATASET_ALL,
            "objects": [encode_dataset_condition(x) for x in var.objects],
        }
    if isinstance(var, DatasetAny):
        return {
            "__type": DAT.DATASET_ANY,
            "objects": [encode_dataset_condition(x) for x in var.objects],
        }
...
def encode_start_trigger_args(var: StartTriggerArgs) -> dict[str, Any]:
    ...
    serialize_kwargs = lambda key: (
        BaseSerialization.serialize(getattr(var, key))
        if getattr(var, key) is not None
        else None
    )
    return {
...
def decode_start_trigger_args(var: dict[str, Any]) -> StartTriggerArgs:
    ...
    :meta private:
    """
    deserialize_kwargs = lambda key: (
        BaseSerialization.deserialize(var[key]) if var[key] is not None else None
    )
...
class BaseSerialization:
    ...
    def serialize(
        ...
        if cls._is_primitive(var):
            ...
            return cls._encode(
                {
                    str(k): cls.serialize(
                        v, strict=strict, use_pydantic_models=use_pydantic_models
                    )
            ...
            return [
                cls.serialize(v, strict=strict, use_pydantic_models=use_pydantic_models)
                for v in var
            ]
        elif (
            ...
            return cls._encode(
                serialized_dataset, type_=serialized_dataset.pop("__type")
            )
        elif isinstance(var, SimpleTaskInstance):
            return cls._encode(
                cls.serialize(
                    var.__dict__, strict=strict, use_pydantic_models=use_pydantic_models
                ),
            ...
            def _pydantic_model_dump(
                model_cls: type[BaseModel], var: Any
            ) -> dict[str, Any]:
    ...
    def deserialize(cls, encoded_var: Any, use_pydantic_models=False) -> Any:
        ...
        if type_ == DAT.TASK_CONTEXT:
            ...
            tr_cls_name, kwargs = cls.deserialize(
                var, use_pydantic_models=use_pydantic_models
            )
...
class DependencyDetector:
    ...
    def detect_dag_dependencies(dag: DAG | None) -> Iterable[DagDependency]:
        ...
        yield from dag.timetable.dataset_condition.iter_dag_dependencies(
...
class SerializedBaseOperator(BaseOperator, BaseSerialization):
    ...
    def serialize_mapped_operator(cls, op: MappedOperator) -> dict[str, Any]:
        serialized_op = cls._serialize_node(
            op, include_deps=op.deps != MappedOperator.deps_for(BaseOperator)
        )
    ...
    @classmethod
    def _serialize_node(
        cls, op: BaseOperator | MappedOperator, include_deps: bool
    ) -> dict[str, Any]:
    ...
    def populate_operator(cls, op: Operator, encoded_op: dict[str, Any]) -> None:
        ...
        for k, v in encoded_op.items():
            ...
            if k == "label":
                ...
                v = {
                    k_: cls.deserialize(v_, use_pydantic_models=True)
    ...
    def deserialize_operator(cls, encoded_op: dict[str, Any]) -> Operator:
        ...
        if encoded_op.get("_is_mapped", False):
            ...
            op_data = {
                k: v
                for k, v in encoded_op.items()
                if k in BaseOperator.get_serialized_fields()
            }
            try:
    ...
    def detect_dependencies(cls, op: Operator) -> set[DagDependency]:
        ...
        def get_custom_dep() -> list[DagDependency]:
            ...
            """
            custom_dependency_detector_cls = conf.getimport(
                "scheduler", "dependency_detector", fallback=None
            )
            if not (
                custom_dependency_detector_cls is None
                or custom_dependency_detector_cls is DependencyDetector
            ):
                warnings.warn(
    ...
    def _deserialize_operator_extra_links(
        ...
        for _operator_links_source in encoded_op_links:
            ...
            if _operator_link_class_path in get_operator_extra_links():
                ...
                in plugins_manager.registered_operator_link_classes
            ):
                ...
                log.error(
                    "Operator Link class %r not registered", _operator_link_class_path
                )
                return {}

            op_link_parameters = {
                param: cls.deserialize(value) for param, value in data.items()
            }
            op_predefined_extra_link: BaseOperatorLink = single_op_link_class(
                **op_link_parameters
            )
    ...
    def _serialize_operator_extra_links(
        ...
        for operator_extra_link in operator_extra_links:
            op_link_arguments = {
                param: cls.serialize(value)
                for param, value in attrs.asdict(operator_extra_link).items()
            }
    ...
    def serialize(
        ...
        return BaseSerialization.serialize(
            var=var, strict=strict, use_pydantic_models=use_pydantic_models
        )

    @classmethod
    def deserialize(cls, encoded_var: Any, use_pydantic_models: bool = False) -> Any:
        return BaseSerialization.deserialize(
            encoded_var=encoded_var, use_pydantic_models=use_pydantic_models
        )
...
class SerializedDAG(DAG, BaseSerialization):
    ...
    def serialize_dag(cls, dag: DAG) -> dict:
        ...
        try:
            ...
            serialized_dag["_task_group"] = TaskGroupSerialization.serialize_task_group(
                dag.task_group
            )

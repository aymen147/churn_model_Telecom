...
def _get_regressor_metrics(y, y_pred, sample_weights):
    sum_on_target = (
        (np.array(y) * np.array(sample_weights)).sum()
...
def _get_aggregate_metrics_values(metrics):
    return {
        name: MetricValue(aggregate_results={name: value})
...
def _extract_output_and_other_columns(model_predictions, output_column_name):
    ...
    if isinstance(model_predictions, list) and all(
        isinstance(p, dict) for p in model_predictions
    ):
        # Extract 'y_pred' and 'other_output_columns' from list of dictionaries
        if output_column_name in model_predictions[0]:
            y_pred = pd.Series(
                [p.get(output_column_name) for p in model_predictions],
                name=output_column_name,
            )
            other_output_columns = pd.DataFrame(
                [
                    {k: v for k, v in p.items() if k != output_column_name}
        ...
        if output_column_name in model_predictions:
            y_pred = pd.Series(
                model_predictions[output_column_name], name=output_column_name
            )
            other_output_columns = pd.DataFrame(
...
def _evaluate_metric(metric_tuple, eval_fn_args):
    ...
    if metric_tuple.index < 0:
        exception_header = (
            f"Did not log builtin metric '{metric_tuple.name}' because it"
        )
    else:
        exception_header = (
    ...
    if scores is not None:
        if not isinstance(scores, list):
            ...
            return
        if any(
            not (_is_numeric(score) or _is_string(score) or score is None)
            for score in scores
        ):
            _logger.warning(
...
def _is_valid_artifacts(artifacts):
    return isinstance(artifacts, dict) and all(
        isinstance(k, str) for k in artifacts.keys()
    )
...
class DefaultEvaluator(ModelEvaluator):
        ...
        try:
            if algorithm:
                if algorithm == "kernel":
                    ...
                    from mlflow.models.evaluation._shap_patch import (
                        _PatchedKernelExplainer,
                    )
        ...
        if self.model_loader_module == "mlflow.sklearn" and self.raw_model is not None:
            try:
                ...
                self.metrics_values.update(
                    _get_aggregate_metrics_values({"score": score})
                )
            except Exception as e:
                _logger.warning(
    ...
    def _log_custom_metric_artifact(
        self, artifact_name, raw_artifact, custom_metric_tuple
    ):
        """
        ...
        mlflow.log_artifact(artifact_file_local_path)
        artifact = inferred_type(
            uri=mlflow.get_artifact_uri(artifact_name + inferred_ext)
        )
        artifact._load(artifact_file_local_path)
        return artifact
    ...
    def _get_args_for_metrics(
        ...
        if (
            len(parameters) == 2
            and param_0_name != "predictions"
            and param_1_name != "targets"
        ):
            ...
            for param_name, param in parameters.items():
                ...
                if (
                    ...
                    # that there is not an error in the dependencies
                        ...
                        metric_tuple.name for metric_tuple in self.ordered_metrics
                    ]:
                ...
                if artifact_results:
                    for artifact_name, raw_artifact in artifact_results.items():
                        self.artifacts[artifact_name] = (
                            self._log_custom_metric_artifact(
                                artifact_name,
                                raw_artifact,
                                custom_artifact_tuple,
                            )
    ...
    def _get_error_message_missing_columns(self, metric_name, param_names):
        ...
        remaining_params = [
            param for param in param_names if param not in special_params
        ]
    ...
    def _raise_exception_for_malformed_metrics(self, malformed_results, eval_df):
        output_columns = (
            []
            if self.other_output_columns is None
            else list(self.other_output_columns.columns)
        )
        if self.predictions:
    ...
    def _log_eval_table(self):
        ...
        if isinstance(self.dataset.features_data, pd.DataFrame):
            ...
            data = pd.DataFrame(
                self.dataset.features_data, columns=self.dataset.feature_names
            )
            if self.dataset.has_targets:
                data = data.assign(
                    **{
        ...
        if (
            self.other_output_columns is not None
            and len(self.other_output_columns_for_eval) > 0
        ):
        ...
        for metric_name, metric_value in self.metrics_values.items():
            ...
            if scores:
                if metric_name.startswith(metric_prefix) and metric_name[
                    len(metric_prefix) :
                ] in [
                ...
                for agg_name, agg_value in metric_value.aggregate_results.items():
                    if agg_value is not None:
                        if agg_name == metric_name.split("/")[0]:
                            ...
                            self.aggregate_metrics[f"{metric_name}/{agg_name}"] = (
                                agg_value
                            )
    ...
    def _evaluate(
        ...
        **kwargs,
        ...
        with TempDir() as temp_dir, matplotlib.rc_context(_matplotlib_config):
            ...
            self.predict_fn = _restrict_langchain_autologging_to_traces_only(
                self.predict_fn
            )
    ...
    def evaluate(
        ...
        **kwargs,
        ...
        self.eval_results_mode = self.evaluator_config.get(
        ...
        baseline_evaluation_result = self._evaluate(

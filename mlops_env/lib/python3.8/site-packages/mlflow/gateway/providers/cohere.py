...
class CohereAdapter(ProviderAdapter):
    ...
    def model_to_chat_streaming(cls, resp, config):
        ...
        return chat.StreamResponsePayload(
            ...
            usage=chat.ChatUsage(
                prompt_tokens=(
                    response["token_count"]["prompt_tokens"] if response else None
                ),
                completion_tokens=(
                    response["token_count"]["response_tokens"] if response else None
                ),
                total_tokens=(
                    response["token_count"]["total_tokens"] if response else None
                ),
            ),
        )
...
class CohereProvider(BaseProvider):
    ...
    def __init__(self, config: RouteConfig) -> None:
        ...
        if config.model.config is None or not isinstance(
            config.model.config, CohereConfig
        ):

...
class SqlAlchemyStore(AbstractStore):
    ...
    def _search_experiments(
        ...
        with self.ManagedSessionMaker() as session:
            ...
            stmt = (
                reduce(
                    lambda s, f: s.join(f), non_attribute_filters, select(SqlExperiment)
                )
    ...
    def _get_experiment(
        self, session, experiment_id, view_type, eager=False
    ):  # noqa: D417
        """
        Args:
        ...
        for run_uuid in run_uuids:
            ...
            for input_uuid, dataset_run_uuid, dataset_sql in datasets:
                if run_uuid == dataset_run_uuid:
                    ...
                    dataset_input_entity = DatasetInput(
                        dataset=dataset_entity, tags=tags
                    )
    ...
    def get_run(self, run_id):
        with self.ManagedSessionMaker() as session:
            ...
            return Run(
                mlflow_run.info, mlflow_run.data, RunInputs(dataset_inputs=inputs)
            )
    ...
    def _get_metric_value_details(self, path, metric):
        _validate_metric(
            metric.key, metric.value, metric.timestamp, metric.step, path=path
        )
    ...
    def _update_latest_metrics_if_necessary(self, logged_metrics, session):
        ...
        # platforms
        metric_key_batches = [
            metric_keys[i : i + 500] for i in range(0, len(metric_keys), 500)
        ]
        for metric_key_batch in metric_key_batches:
        ...
        for logged_metric in logged_metrics:
            ...
            # metric comparison is successful.
                ...
                if _compare_metrics(logged_metric, new_latest_metric):
                    new_latest_metric = _overwrite_metric(
    ...
    def _search_runs(
        ...
        with self.ManagedSessionMaker() as session:
            ...
            cases_orderby, parsed_orderby, sorting_joins = _get_orderby_clauses(
                order_by, session
            )
    ...
    def _get_trace_artifact_location_tag(
        self, experiment, request_id: str
    ) -> SqlTraceTag:
    ...
    def end_trace(
        ...
        with self.ManagedSessionMaker() as session:
            ...
            for k, v in request_metadata.items():
                session.merge(
                    SqlTraceRequestMetadata(request_id=request_id, key=k, value=v)
                )
    ...
    def search_traces(
        ...
        with self.ManagedSessionMaker() as session:
            cases_orderby, parsed_orderby, sorting_joins = (
                _get_orderby_clauses_for_search_traces(order_by or [], session)
            )
        ...
        if SearchUtils.is_string_attribute(
            ...
            if entity == SqlDataset:
                if key_name == "context":
                    dataset_filters.append(
                        ...
                        .join(
                            ...
                            and_(
                                ...
                                SqlInputTag.name == MLFLOW_DATASET_CONTEXT,
                                SearchUtils.get_sql_comparison_func(
                                    comparator, dialect
                    ...
                    dataset_attr_filter = SearchUtils.get_sql_comparison_func(
                        comparator, dialect
...
def _get_search_experiments_order_by_clauses(order_by):
    ...
    return [
        col.asc() if ascending else col.desc() for col, ascending in order_by_clauses
    ]
...
def _get_filter_clauses_for_search_traces(filter_string, session, dialect):
    ...
    parsed_filters = SearchTraceUtils.parse_search_filter_for_search_traces(
        filter_string
    )

...
def start_run(
    ...
    if existing_run_id:
        ...
        if parent_run_id:
            ...
            if (
                len(_active_run_stack) > 0
                and parent_run_id != _active_run_stack[-1].info.run_id
            ):
            ...
            parent_run_id = (
                _active_run_stack[-1].info.run_id
    ...
    if log_system_metrics:
        ...
        try:
            ...
            run_id_to_system_metrics_monitor[active_run_obj.info.run_id] = (
                system_monitor
            )
...
def end_run(status: str = RunStatus.to_string(RunStatus.FINISHED)) -> None:
    ...
    if len(_active_run_stack) > 0:
        ...
        if _last_active_run_id in run_id_to_system_metrics_monitor:
            system_metrics_monitor = run_id_to_system_metrics_monitor.pop(
                _last_active_run_id
            )
...
def log_dict(
    dictionary: Dict[str, Any], artifact_file: str, run_id: Optional[str] = None
) -> None:
    """
...
def log_image(
    ...
    MlflowClient().log_image(
        run_id, image, artifact_file, key, step, timestamp, synchronous
    )
...
def autolog(
    ...
    def setup_autologging(module):
        try:
            ...
            autolog_module = importlib.import_module(
                LIBRARY_TO_AUTOLOG_MODULE[module.__name__]
            )
            ...
            if is_testing():
                ...
                raise
            elif autologging_params is None or not autologging_params.get(
                "silent", False
            ):
                _logger.warning(
    ...
    for module in list(
        set(LIBRARY_TO_AUTOLOG_MODULE.keys()) - {"pyspark", "pyspark.ml"}
    ):

    ...
    if scheme in ("file", ""):
        if not os.path.exists(path):
            raise MlflowException.invalid_parameter_value(
                f"{allowlist_file} does not exist"
            )
...
def _read_log_model_allowlist():
    ...
    if sys.version_info.major > 2 and sys.version_info.minor > 8:
        from importlib.resources import (
            as_file,
            files,
        )  # clint: disable=lazy-builtin-import
...
def _gen_stage_hierarchy_recursively(stage, uid_to_indexed_name_map):
    ...
    if isinstance(stage, Pipeline):
        ...
        for sub_stage in stage.getStages():
            sub_hierarchy = _gen_stage_hierarchy_recursively(
                sub_stage, uid_to_indexed_name_map
            )
        ...
        for param_name, param_value in _get_stage_type_params(stage).items():
            sub_hierarchy = _gen_stage_hierarchy_recursively(
                param_value, uid_to_indexed_name_map
            )
...
def _gen_estimator_metadata(estimator):
    ...
    param_search_estimators = [
        stage
        for stage in _traverse_stage(estimator)
        if _is_parameter_search_estimator(stage)
    ]
...
def _get_stage_type_params(instance):
    ...
    return {
        ...
        for param in instance.params
        if instance.isDefined(param)
        and isinstance(instance.getOrDefault(param), Params)
    }
...
def _get_instance_param_map_recursively(instance, level, uid_to_indexed_name_map):
    ...
    logged_param_name_prefix = (
        "" if level == 0 else uid_to_indexed_name_map[instance.uid] + "."
    )
    ...
    for param_name, param_value in param_map.items():
        ...
        if is_pipeline and param_name == "stages":
            ...
            expanded_param_map[logged_param_name] = uid_to_indexed_name_map[
                param_value.uid
            ]
...
def _create_child_runs_for_parameter_search(
    parent_estimator, parent_model, parent_run, child_tags
):
    client = MlflowClient()
    ...
    for i, est_param in enumerate(estimator_param_maps):
        ...
        for params_batch, metrics_batch in zip_longest(
            ...
            client.log_batch(
                ...
                metrics=[
                    Metric(
                        key=str(key), value=value, timestamp=child_run_end_time, step=0
                    )
...
def _get_param_search_metrics_and_best_index(
    param_search_estimator, param_search_model
):
    """
    ...
    if isinstance(param_search_model, CrossValidatorModel):
        ...
        raise RuntimeError(
            f"Unknown parameter search model type {type(param_search_model)}."
        )
...
class _AutologgingMetricsManager:
    ...
    def should_log_post_training_metrics(self):
        ...
        """
        return (
            not _SparkTrainingSession.is_active()
    ...
    def register_evaluator_call(
        self, run_id, metric_name, dataset_name, evaluator_info
    ):
        """
...
def _check_or_set_model_prediction_column(spark_model, input_spark_df):
    ...
    if isinstance(spark_model, PipelineModel) and spark_model.stages[-1].hasParam(
        "outputCol"
    ):
        from mlflow.utils._spark_utils import _get_active_spark_session
        ...
        transformed_df = spark_model.transform(
            spark.createDataFrame([], input_spark_df.schema)
        )
...
def _infer_spark_model_signature(spark_model, input_example_spark_df):
    ...
    prediction_column = _check_or_set_model_prediction_column(
        spark_model, input_example_spark_df
    )
    model_output = spark_model.transform(input_example_spark_df).select(
        prediction_column
    )
...
def autolog(
    ...
    **How does autologging work for meta estimators?**
    ...
    def _log_pretraining_metadata(estimator, params, input_df):
        ...
        param_map = _get_instance_param_map(
            estimator, autologging_metadata.uid_to_indexed_name_map
        )
        ...
        if log_models:
            if _should_log_model(spark_model):
                ...
                def _get_input_example_spark_df():
                    ...
                    return input_df.select(feature_cols).limit(
                        INPUT_EXAMPLE_SAMPLE_ROWS
                    )

                def _infer_model_signature(input_example_slice):
                    return _infer_spark_model_signature(
                        spark_model, input_example_slice
                    )

                # TODO: Remove this once we support non-scalar spark data types
                nonlocal log_model_signatures
                if log_model_signatures:
                    if unsupported_columns := _get_columns_with_unsupported_data_type(
                        input_df
                    ):
                        _logger.warning(
    ...
    def patched_transform(original, self, *args, **kwargs):
        ...
        if _AUTOLOGGING_METRICS_MANAGER.should_log_post_training_metrics() and run_id:
            ...
            eval_dataset_name = (
                _AUTOLOGGING_METRICS_MANAGER.register_prediction_input_dataset(
                    self, eval_dataset
                )
            )

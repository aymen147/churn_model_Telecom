...
class APIRequest:
    ...
    def single_call_api(self, callback_handlers: Optional[List[BaseCallbackHandler]]):
        ...
        if isinstance(self.lc_model, BaseRetriever):
            ...
            if (
                isinstance(self.lc_model, langchain.chains.base.Chain)
                and not self.stream
            ):
            ...
            response = self._predict_single_input(
                self.request_json, callback_handlers, **kwargs
            )
    ...
    def call_api(
        ...
        """
        _logger.debug(
            f"Request #{self.index} started with payload: {self.request_json}"
        )
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        while True:
            ...
            if not retry_queue.empty():
                ...
                _logger.warning(
                    f"Retrying request {next_request.index}: {next_request}"
                )

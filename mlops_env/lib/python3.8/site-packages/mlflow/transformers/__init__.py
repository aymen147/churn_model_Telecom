...
_PEFT_PIPELINE_ERROR_MSG = re.compile(
    r"The model 'PeftModel[^']*' is not supported for"
)
...
def _validate_transformers_model_dict(transformers_model):
    ...
    if isinstance(transformers_model, dict):
        invalid_keys = [
            key for key in transformers_model.keys() if key not in _SUPPORTED_SAVE_KEYS
        ]
        if invalid_keys:
...
def save_model(
    ...
    **kwargs,  # pylint: disable=unused-argument
    ...
    if isinstance(transformers_model, transformers.Pipeline):
        ...
        built_pipeline = DummyPipeline(
            task=task, model=DummyModel(name_or_path=transformers_model)
        )
    else:
        raise MlflowException(
    ...
    if input_example is not None:
        input_example = format_input_example_for_special_cases(
            input_example, built_pipeline
        )
    ...
    if (
        ...
        and (
            metadata_task := mlflow_model.metadata.get(_METADATA_LLM_INFERENCE_TASK_KEY)
        )
    ...
    if not save_pretrained and not is_valid_hf_repo_id(
        built_pipeline.model.name_or_path
    ):
        _logger.warning(
    ...
    if isinstance(transformers_model, str):
        ...
        flavor_conf = build_flavor_config(
            built_pipeline, processor, torch_dtype, save_pretrained
        )
    ...
    if llm_inference_task:
        ...
        if mlflow_model.metadata:
            ...
            mlflow_model.metadata = {
                _METADATA_LLM_INFERENCE_TASK_KEY: llm_inference_task
            }
    ...
    if save_pretrained:
        if isinstance(transformers_model, str):
            ...
            save_pipeline_pretrained_weights(
                path, built_pipeline, flavor_conf, processor
            )
    else:
    ...
    if (
        ...
        isinstance(transformers_model, str)
    ...
    if conda_env is None:
        if pip_requirements is None:
            ...
            if isinstance(transformers_model, str) or is_peft_model(
                built_pipeline.model
            ):
                _logger.info(
...
def persist_pretrained_model(model_uri: str) -> None:
    ...
    file_names = [
        os.path.basename(f.path) for f in artifact_repo.list_artifacts(artifact_path)
    ]
    ...
    with TempDir() as tmp_dir:
        local_model_path = artifact_repo.download_artifacts(
            artifact_path, dst_path=tmp_dir.path()
        )
        ...
        for dir_to_upload in (_MODEL_BINARY_FILE_NAME, _COMPONENTS_BINARY_DIR_NAME):
            ...
            try:
                artifact_repo.log_artifacts(
                    local_dir, os.path.join(artifact_path, dir_to_upload)
                )
            except Exception as e:
    ...
    _logger.info(
        f"The pretrained model has been successfully persisted in {model_uri}."
    )
...
def _load_model(path: str, flavor_config, return_type: str, device=None, **kwargs):
    ...
    if dtype_val := kwargs.get(_TORCH_DTYPE_KEY) or flavor_config.get(
        FlavorKey.TORCH_DTYPE
    ):
    ...
    accelerate_model_conf["low_cpu_mem_usage"] = (
        MLFLOW_HUGGINGFACE_USE_LOW_CPU_MEM_USAGE.get()
    )
    ...
    if return_type == "pipeline":
        conf.update(**kwargs)
        with suppress_logs(
            "transformers.pipelines.base", filter_regex=_PEFT_PIPELINE_ERROR_MSG
        ):
...
def _fetch_model_card(model_name):
    ...
    if hasattr(hub, "ModelCard"):
        try:
            ...
            _logger.warning(
                f"The model card could not be retrieved from the hub due to {e}"
            )
    else:
        _logger.warning(
...
def _write_card_data(card_data, path):
    ...
    if card_data:
        try:
            path.joinpath(_CARD_TEXT_FILE_NAME).write_text(
                card_data.text, encoding="utf-8"
            )
...
def _write_license_information(model_name, card_data, path):
    ...
    if license_file := _extract_license_file_from_repository(model_name):
        try:
            ...
            license_location = hub.hf_hub_download(
                repo_id=model_name, filename=license_file
            )
        except Exception as e:
...
def _build_pipeline_from_model_input(
    model_dict: Dict[str, Any], task: Optional[str]
) -> Pipeline:
    """
    ...
    if not (
        isinstance(model, _get_supported_pretrained_model_types())
...
def _validate_llm_inference_task_type(
    llm_inference_task: str, pipeline_task: str
) -> None:
    """
    Validates that an ``inference_task`` type is supported by ``transformers`` pipeline type.
    """
    supported_llm_inference_tasks = (
        _SUPPORTED_LLM_INFERENCE_TASK_TYPES_BY_PIPELINE_TASK.get(pipeline_task, [])
    )
...
def _load_pyfunc(path, model_config: Optional[Dict[str, Any]] = None):
    ...
    model_config = _get_model_config(
        local_path.joinpath(_COMPONENTS_BINARY_DIR_NAME), model_config
    )
...
@experimental
def generate_signature_output(
    pipeline, data, model_config=None, params=None, flavor_config=None
):
    """
...
class _TransformersWrapper:
    def __init__(
        self, pipeline, flavor_config=None, model_config=None, prompt_template=None
    ):
        ...
        self.llm_inference_task = (
            self.flavor_config.get(_LLM_INFERENCE_TASK_KEY)
    ...
    def _validate_model_config_and_return_output(
        self, data, model_config, return_tensors=False
    ):
        import transformers
        ...
        try:
            ...
            if isinstance(
                ...
                # transformers > 4.33.3
                or "Soundfile is either not in the correct format or is malformed"
                in str(e)
            ):
                raise MlflowException.invalid_parameter_value(
    ...
    def predict(self, data, params: Optional[Dict[str, Any]] = None):
        ...
        if self.llm_inference_task == _LLM_INFERENCE_TASK_CHAT:
            data, params = preprocess_llm_inference_input(
                data, params, self.flavor_config
            )
            data = [
                convert_messages_to_prompt(msgs, self.pipeline.tokenizer)
                for msgs in data
            ]
        ...
        if isinstance(input_data, dict):
            ...
            isinstance(entry, dict) for entry in input_data
        ):
    ...
    def _strip_input_from_response_in_instruction_pipelines(
        ...
        def trim_input(data_in, data_out):
            ...
            if (
                not include_prompt
                and flavor_config[FlavorKey.INSTANCE_TYPE]
                in self._supported_custom_generator_types
        ...
        if isinstance(input_data, list) and isinstance(output, list):
            return [
                trim_input(data_in, data_out)
                for data_in, data_out in zip(input_data, output)
            ]
    ...
    def _parse_lists_of_dict_to_list_of_str(
        self, output_data, target_dict_key
    ) -> List[str]:
        """
                ...
                if isinstance(output, dict):
                    for key, value in output.items():
                        if key == target_dict_key:
                            ...
                            output_coll.extend(
                                self._parse_lists_of_dict_to_list_of_str(
                                    value, target_dict_key
                                )
                            )
                elif isinstance(output, list):
                    output_coll.extend(
                        self._parse_lists_of_dict_to_list_of_str(
                            output, target_dict_key
                        )
                    )
            return output_coll
    ...
    def _parse_tokenizer_output(self, output_data, target_set):
        ...
        if isinstance(output_data[0], list):
            return [
                self._parse_tokenizer_output(coll, target_set) for coll in output_data
            ]
        else:
    ...
    def _parse_list_of_multiple_dicts(output_data, target_dict_key):
        ...
        if isinstance(output_data[0], list):
            return [
                fetch_target_key_value(collection, target_dict_key)
    ...
    def _parse_json_encoded_list(self, data, key_to_unpack):
        ...
        if isinstance(data, list):
            return [
                self._parse_json_encoded_list(entry, key_to_unpack) for entry in data
            ]
        elif isinstance(data, dict):
            ...
            if isinstance(data[key_to_unpack], str):
                try:
                    return {
                        k: (json.loads(v) if k == key_to_unpack else v)
    ...
    def _validate_str_or_list_str(data):
        ...
        elif isinstance(data, list) and not all(
...
def _validate_prompt_template(prompt_template):
    ...
    format_args = [
        tup[1]
        for tup in string.Formatter().parse(prompt_template)

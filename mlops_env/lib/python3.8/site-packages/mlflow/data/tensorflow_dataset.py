...
from mlflow.data.pyfunc_dataset_mixin import (
...
class TensorFlowDataset(Dataset, PyFuncConvertibleDatasetMixin):
    ...
    def _compute_digest(self) -> str:
        ...
        if isinstance(self._features, tf.data.Dataset):
            return self._compute_tensorflow_dataset_digest(
    ...
    def profile(self) -> Optional[Any]:
        ...
        profile = {
            "features_cardinality": (
                int(self._features.cardinality().numpy())
                if isinstance(self._features, tf.data.Dataset)
    ...
    def schema(self) -> Optional[TensorDatasetSchema]:
        ...
        try:
            ...
            _logger.warning(
                "Failed to infer schema for TensorFlow dataset. Exception: %s", e
            )
            return None
    ...
    def _get_tf_object_schema(tf_object) -> Schema:
        ...
        if isinstance(tf_object, tf.data.Dataset):
            ...
            if isinstance(numpy_data, np.ndarray):
                ...
                return TensorFlowDataset._get_schema_from_tf_dataset_dict_numpy_data(
                    numpy_data
                )
            elif isinstance(numpy_data, tuple):
                return TensorFlowDataset._get_schema_from_tf_dataset_tuple_numpy_data(
                    numpy_data
                )
            else:
                raise MlflowException(
    ...
    @staticmethod
    def _get_schema_from_tf_dataset_dict_numpy_data(
        numpy_data: Dict[Any, Any]
    ) -> Schema:
        if not all(
            isinstance(data_element, np.ndarray) for data_element in numpy_data.values()
        ):
            raise MlflowException(
    ...
    def to_evaluation_dataset(self, path=None, feature_names=None) -> EvaluationDataset:
        ...
        if not tf.is_tensor(self._features):
            raise MlflowException(
                "Data must be a Tensor to convert to an EvaluationDataset."
            )
        if self._targets is not None and not tf.is_tensor(self._targets):
            raise MlflowException(
                "Targets must be a Tensor to convert to an EvaluationDataset."
            )
